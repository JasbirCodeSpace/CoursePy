...
class Coupons(Spider):
	...
	def parse_comidoc_each_page(self,response):
		items = UdemyCouponsItem()
		items['site'] = 'Comidoc'
		items['name'] = response.xpath('//*[@id="__next"]/div/div/div/div[1]/h1/text()').extract_first()
		...
		items['link'] = 'https://www.udemy.com/course/'+[s for s in temp if "cleanUrl" in s][0].split(':')[-1].strip('"').strip('}').strip('"')+'?couponCode='+items['code']
		# print("comidoc",items)
		return items
	...
	def parse_learnviral(self,response):
		all_items = response.css('.entry-title a::attr(href)').extract()
		for url in all_items:
			yield Request(url,callback=self.parse_learnviral_each_page)

	def parse_learnviral_each_page(self,response):
		items = UdemyCouponsItem()
		items['site'] = 'Learnviral'
		items['name'] = response.css('.entry-title a::text').extract_first()
		items['link'] = response.css('a.promotion::attr(href)').extract_first()
		items['tags'] = response.css('p.meta a::text').extract()
		parameters = parse_qs(urlparse.urlparse(items['link']).query)
		items['code'] = parameters['couponCode'][0] if 'couponCode' in parameters else 'N/A'
		return items
		
	def parse_realdiscount(self,response):
		all_items = response.css('.embed-responsive a::attr(href)').extract()
		for url in all_items:
			yield Request(url,callback=self.parse_realdiscount_each_page)

	def parse_realdiscount_each_page(self,response):
		items = UdemyCouponsItem()
		items['site'] = 'RealDiscount'
		items['name'] = response.css('.breadcrumb_last::text').extract_first()
		items['link'] = response.css('div.deal-sidebar-box div.letshover a::attr(href)').extract_first()
		items['tags'] = response.css('.tags-list1 div a::text').extract()
		parameters = parse_qs(urlparse.urlparse(items['link']).query)
		items['code'] = parameters['couponCode'][0] if 'couponCode' in parameters else 'N/A'
		return items
